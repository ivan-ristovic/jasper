...
function demoSnapshot() {
  const actions = [
    {
      ...
      argsSchema: [
        ...
        { name: 'serializerTag', label: 'Serializer tag', type: 'jmh-param', default: 'java', required: false, help: 'Optional serTag override.' },
        { name: 'workloadTag', label: 'Workload tag', type: 'jmh-param', default: 'lst<int>[1]', required: false, help: 'Optional objTag override.' },
        { name: 'warmupIterations', label: 'Warmup iterations', type: 'integer', default: 0, required: false, min: 0, max: 1000 },
        { name: 'iterations', label: 'Measurement iterations', type: 'integer', default: 1, required: false, min: 1, max: 1000 },
        { name: 'warmupTime', label: 'Warmup time', type: 'duration', default: '100ms', required: false },
        { name: 'measurementTime', label: 'Measurement time', type: 'duration', default: '100ms', required: false },
        { name: 'forks', label: 'Forks', type: 'integer', default: 1, required: false, min: 0, max: 1000 }
      ]
    },
    {
      id: 'sd-baselines-run-all',
  ...
  const running = {
    id: 'demo-running',
    actionId: 'sd-baselines-run-all',
    actionLabel: 'All memory syscall demos',
    benchmark: 'sd-baselines',
    status: 'running',
    createdAt: '2026-06-18T10:22:00.000Z',
    startedAt: '2026-06-18T10:22:04.000Z',
    commandPreview: 'microbenchmarks/sd-baselines/run_all.sh "100000"',
    durationMs: null
  };
  const done = {
    id: 'demo-succeeded',
    actionId: 'mn-cache-isolate-bench-rss',
    ...
    durationMs: 89000,
    exitCode: 0,
    commandPreview: 'macrobenchmarks/mn-cache-isolate/bench-rss "8" "5s" "y"'
  };

  const logs = new Map();
  logs.set('demo-running', [
    '$ microbenchmarks/sd-baselines/run_all.sh "100000"',
    'cwd: microbenchmarks/sd-baselines',
    '',
    'Entering memcpy/',
  ...
  logs.set('demo-succeeded', [
    '$ macrobenchmarks/mn-cache-isolate/bench-rss "8" "5s" "y"',
...
async function init() {
  ...
  if (demoMode) {
    ...
    if (demoMode === 'config') state.selectedActionId = 'mn-cache-isolate-bench-rss';
    render();
    return;
  }
