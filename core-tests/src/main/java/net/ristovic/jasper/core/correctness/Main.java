...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        Serializer serializer = new SampleSerializer();
        testCorrectness(serializer);
        testRobustness(serializer);
    }
    ...
    private static File createZipFile() throws IOException {
        Path zip = Files.createTempFile("jasper-core-tests", ".zip");
        try (ZipOutputStream out = new ZipOutputStream(Files.newOutputStream(zip))) {
            out.putNextEntry(new ZipEntry("empty"));
            out.write(new byte[] { 1 });
            out.closeEntry();
        }
        File file = zip.toFile();
        file.deleteOnExit();
        return file;
    }
